...
权限管理页面是整个 RBAC 体系的**唯一权限配置工作台**，负责两件事：
...
**与角色管理的区别**：角色管理（Role-Management.md）**仅负责维护角色本身（增删改查、启用/禁用等）**，不再在本页内直接配置菜单/接口权限；列表中的"配置权限"按钮作为跳转入口，打开本权限管理页面并自动定位到对应角色与平台。本页面是**唯一的权限配置工作台**，提供完整上下文：菜单树全量可见、操作级权限内联展示、API 绑定状态直观呈现，适合系统初始化或大批量权限配置场景。
...
> - 角色管理页面不直接编辑权限数据范围，仅通过"配置权限"按钮跳转到本页，由本页统一处理。
...
| 角色-菜单权限配置 | 权限管理（本页） | 所有角色菜单/操作权限均在本页集中配置；角色管理仅提供跳转入口 |
...
| 模块 | 关联关系 |
|---|---|
| 角色管理（Role-Management） | 通过"配置权限"按钮跳转到本页并预选中对应角色与平台，本页负责具体权限配置 |
